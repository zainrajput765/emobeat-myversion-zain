...
try:
    ...
    for i in range(5):
        # Generate random dummy image (simulating different inputs)
        ...
        with torch.no_grad():
            ...
            print(f"Random Image {i+1} -> Predicted: {EMOTION_LABELS[pred.item()]} ({conf.item()*100:.1f}%) | Raw index: {pred.item()}")
